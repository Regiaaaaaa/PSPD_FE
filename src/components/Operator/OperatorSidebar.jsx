...
export default function OperatorSidebar({ isOpen, onClose }) {
  ...
  const menuItems = [
    ...
    { path: "/operator/books", label: "Books", icon: BookOpen },
    { path: "/operator/borrow", label: "Peminjaman", icon: BookMarked },
    { path: "/operator/returns", label: "Pengembalian", icon: BookCheck },
  ];
      ...
      >
        ...
        <div className={`h-14 flex items-center border-b border-gray-200 ${(isExpanded || isOpen) ? 'justify-between px-3' : 'justify-center px-2'}`}>
          <div className={`flex items-center gap-2 min-w-0 ${(!isExpanded && !isOpen) && 'justify-center'}`}>
            ...
            />
            {(isExpanded || isOpen) && (
              <span className="font-semibold text-gray-800 text-sm truncate">Operator Panel</span>
            )}
          </div>
