...
const CreateBook = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.harga_buku !== '' && isNaN(formData.harga_buku)) {
      toast.error('Harga buku harus berupa angka');
      return;
    }
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                ...
                <div className="form-control md:col-span-2">
                  <label className="label">
                    <span className="label-text text-xs sm:text-sm font-medium">Judul Buku <span className="text-error">*</span></span>
                  </label>
                  <input type="text" name="judul" placeholder="Masukkan judul buku"
                    ...
                    value={formData.judul} onChange={handleInputChange} required disabled={loading} />
                </div>

                <div className="form-control">
                  <label className="label">
                    <span className="label-text text-xs sm:text-sm font-medium">ISBN <span className="text-error">*</span></span>
                  </label>
                  <input type="text" name="isbn" placeholder="Masukkan ISBN (10-13 digit)" maxLength="13"
                    className="input input-sm sm:input-md input-bordered bg-white w-full"
                    value={formData.isbn} onChange={handleInputChange} required disabled={loading} />
                  <label className="label">
                    <span className="label-text-alt text-xs text-gray-500">Hanya angka, 10-13 digit</span>
                  </label>
                </div>

                <div className="form-control">
                  <label className="label">
                    <span className="label-text text-xs sm:text-sm font-medium">Kategori <span className="text-error">*</span></span>
                  </label>
                  <select multiple name="kategori_id"
                    className="select select-sm sm:select-md select-bordered bg-white w-full h-32"
                    value={formData.kategori_id} onChange={handleKategoriChange} disabled={loading}>
                    {categories.map(cat => (
                ...
                <div className="form-control">
                  ...
                  <input type="text" name="penulis" placeholder="Masukkan nama penulis"
                    className="input input-sm sm:input-md input-bordered bg-white w-full"
                    value={formData.penulis} onChange={handleInputChange} disabled={loading} />
                </div>
                ...
                <div className="form-control">
                  ...
                  <input type="text" name="penerbit" placeholder="Masukkan penerbit"
                    className="input input-sm sm:input-md input-bordered bg-white w-full"
                    value={formData.penerbit} onChange={handleInputChange} disabled={loading} />
                </div>
                ...
                <div className="form-control">
                  ...
                  <input type="text" name="tahun_terbit" placeholder="2024" maxLength="4"
                    className="input input-sm sm:input-md input-bordered bg-white w-full"
                    value={formData.tahun_terbit} onChange={handleInputChange} disabled={loading} />
                </div>

                <div className="form-control">
                  <label className="label">
                    <span className="label-text text-xs sm:text-sm font-medium">Stok Total <span className="text-error">*</span></span>
                  </label>
                  <input type="number" name="stok_total" placeholder="0" min="0"
                    className="input input-sm sm:input-md input-bordered bg-white w-full"
                    value={formData.stok_total} onChange={handleInputChange} required disabled={loading} />
                </div>

                <div className="form-control">
                  <label className="label">
                    <span className="label-text text-xs sm:text-sm font-medium">Harga Buku (Rp)</span>
                  </label>
                  <input type="number" name="harga_buku" placeholder="0" min="0"
                    className="input input-sm sm:input-md input-bordered bg-white w-full"
                    value={formData.harga_buku} onChange={handleInputChange} disabled={loading} />
                  <label className="label">
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
              <button type="button" className="btn btn-sm sm:btn-md btn-ghost order-2 sm:order-1"
                onClick={() => navigate('/admin/books')} disabled={loading}>Batal</button>
              <button type="submit" className="btn btn-sm sm:btn-md bg-blue-600 hover:bg-blue-700 text-white border-none order-1 sm:order-2" disabled={loading}>
                {loading ? <><span className="loading loading-spinner loading-xs sm:loading-sm"></span>Menyimpan...</> : 'Simpan Buku'}
              </button>
            </div>
          </form>
        </div>
